...
export function estimateTrip({ segments, payloadKg, battery_pct, tariff, params }) {
  ...
  const perSeg = segments.map((s) => {
    ...
    const sinT = Math.max(-0.3, Math.min(0.3, s.delta_h / Math.max(s.distance_m, 1))); // road grade, clamped
    ...
    let e =
      F >= 0
        ? (F * s.distance_m) / p.eta_dt / 3600 // drivetrain loss, Wh
        : F * s.distance_m * p.eta_regen * p.eta_dt / 3600; // regen recovery, Wh (negative)
  ...
  const kWh_per_km = total_kWh / dist_km;

  const predicted_full_range_km = p.battery_kWh / kWh_per_km; // range on a full battery
  const arrival_soc_pct = ((p.battery_kWh * (battery_pct / 100) - total_kWh) / p.battery_kWh) * 100;
